...
class PacoteDAOImplTeste {
	...
	void buscarPorDatas() {
		...
		for(Pacote p:lista) {
			assertTrue(p.getDataSaida().after(d1) && p.getDataSaida().before(d2));
		}
		
	}
	
	@Test
	 void somadeprecosDePacotePorTransporte() {
		Transporte t = transportedao.pesquisar(2);
		double result =pacoteDAO.somaPrecosPacotesPorTransporte(t);
